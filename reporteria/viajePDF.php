...
$y = $y + 10;
$pdf->SetXY($x + 30, $y);
$pdf->Cell(40, 10, utf8_decode($nombreCiudadOrigen . ', ' . $nombreDeptoOrigen . ', ' . $nombrePaisOrigen));

$y = $y + 10;
$pdf->SetXY($x + 30, $y);
$pdf->Cell(40, 10, utf8_decode($nombreCiudadDestino . ', ' . $nombreDeptoDestino . ', ' . $nombrePaisDestino));
...
$consulta = "SELECT Id_Boleto, Id_Cliente, Nombre FROM v_Boleto WHERE Id_Viaje = '$idViaje' ORDER BY Id_Boleto";
...
if (mysqli_num_rows($resultado) != 0) {
    ...
    $pdf->Cell(35, 10, 'No. Boleto', 1, 0, 'C', 0);
    $pdf->Cell(50, 10, 'Identidad del Cliente', 1, 0, 'C', 0);
    $pdf->Cell(100, 10, 'Nombre del Cliente', 1, 1, 'C', 0);


    $pdf->SetFont('Arial', '', 11);
    while ($row = $resultado->fetch_assoc()) {
        $pdf->Cell(6);
        $pdf->Cell(35, 10, utf8_decode($row["Id_Boleto"]), 1, 0, 'C', 0);
        $pdf->Cell(50, 10, utf8_decode($row["Id_Cliente"]), 1, 0, 'C', 0);
        $pdf->Cell(100, 10, utf8_decode($row["Nombre"]), 1, 1, 'C', 0);
    }
    $x = $pdf->GetX();
    $y = $pdf->GetY();
}
